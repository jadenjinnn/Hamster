...
- `Hamster-Py/src/` — pybind11 bindings: `main.cpp` (module entry point), `HamsterBehaviour.h` (trampoline + binding; adds `find_entity_by_name`, `animate`/`stop_animation`, parent/child API), `EntityHandle.h` (runtime entity handle: `add_component`, `set_texture`/`set_label`/`set_text`/`set_visible`, `transform`, `set_velocity`, `apply_impulse`, parent/children), `Components.h` (Transform/Sprite/Rigidbody/BodyType/ColliderShape bindings), `Library.h` (vec2/vec3/vec4), `Core.h` (Scene/Application/EventDispatcher — opaque; `Scene.find_entity_by_name`), `Input.h` (KeyCodes enum), `UUID.h`, `Log.h`
...
7. **`Scene::OnUpdate()`** on the active scene (if any, and only when simulation is running):
   ...
   - `ProcessContactEvents()`: reads `b2World_GetContactEvents`, resolves entity UUIDs via `b2Body_GetUserData`, posts `CollisionEvent` on begin-touch and `CollisionEndEvent` on end-touch. `HamsterBehaviour` adds/removes the partner from its collision set on each, so `self.colliding` reflects *current* contact state rather than latching true on first contact. Box2D emits no end-touch when a body is destroyed, so `DestroyEntity` synthesises end-touch for the dying body's live contacts (see `Scene::DestroyEntity`).
   ...
   - Animation advance: iterates entities with `Animation` + `Sprite`, advances `currentTime`, points `Sprite::assetUUID` at the current keyframe's asset (whole texture **or** spritesheet sub-sprite — the renderer resolves the UV rect via `ResolveSpriteSource`), posts `AnimationCompletedEvent` for non-looping animations that finish
...
C++ drives Python (not the reverse). Engine calls `on_create` once and `on_update` every simulation tick, plus event callbacks (`on_collision`, `on_animation_complete`, `on_button_clicked`). Python calls back into C++ through `HamsterBehaviour` methods (`self.transform`, `self.key_pressed`, `self.log(...)`, `self.apply_force/apply_impulse/set_velocity`, `self.animate(...)`, `self.find_entity_by_name(...)`, `self.create_entity/destroy_entity`) and through the `EntityHandle` returned by entity lookups/creation (`set_texture`, `set_label`, `set_text`, `set_visible`, `transform`, parent/children).
...
Scene::OnUpdate [if simulation running]
  ...
  ├─ ProcessContactEvents: b2World_GetContactEvents → resolve UUID via userData → post CollisionEvent (begin) / CollisionEndEvent (end)
