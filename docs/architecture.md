...
- `Hamster-Core/src/Renderer/` — `Renderer` (instance owned by Application, OpenGL draw calls, camera/zoom; same-texture **sprite batching** via `BeginSpriteBatch`/`SubmitSprite`/`EndSpriteBatch`, and an anchored **UI pass** via `BeginUIPass`/`SubmitUIRect`/`SubmitUIText`/`EndUIPass` + `ResolveUIButton`), `Shader`, `Texture`, `FramebufferTexture`, `FontAtlas` (bakes Inter-Regular into an R8 glyph atlas for UI text), built-in GLSL shaders (`SpriteShader`, `FlatShader`, `SpriteBatchShader`, `UIRectShader`, `UITextShader`)
